...
##缶の検出
    ...
    def initImageLogger(self,save_name,is_color):
        ...
        self.log_image_writer = cv2.VideoWriter(save_path,img_format,self.fps)
...
##ここまで
    ...
    hsv_max_limit = [20, 255, 255]
    hsv_min_limit = [0, 110, 120]

    cap = cv2.VideoCapture(0)

    try:
        cv2.namedWindow("HSV")
        cv2.createTrackbar("Hmax", "HSV", 180, 360, nothing)
        cv2.createTrackbar("Smax", "HSV", 180, 360, nothing)
        cv2.createTrackbar("Vmax", "HSV", 180, 360, nothing)
        cv2.createTrackbar("Hmin", "HSV", 0, 360, nothing)
        cv2.createTrackbar("Smin", "HSV", 0, 360, nothing)
        cv2.createTrackbar("Vmin", "HSV", 0, 360, nothing)
        cv2.createTrackbar("H2max", "HSV", 180, 360, nothing)
        cv2.createTrackbar("H2min", "HSV", 0, 360, nothing)

        while True:
            hsv_max_limit[0] = cv2.getTrackbarPos("Hmax", "HSV")
            hsv_max_limit[1] = cv2.getTrackbarPos("Smax", "HSV")
            hsv_max_limit[2] = cv2.getTrackbarPos("Vmax", "HSV")
            hsv_min_limit[0] = cv2.getTrackbarPos("Hmin", "HSV")
            hsv_min_limit[1] = cv2.getTrackbarPos("Smin", "HSV")
            hsv_min_limit[2] = cv2.getTrackbarPos("Vmin", "HSV")
            hsv_min = (hsv_min_limit[0], hsv_min_limit[1], hsv_min_limit[2])
            hsv_max = (hsv_max_limit[0], hsv_max_limit[1], hsv_max_limit[2])

            ret, frame = cap.read()
            hsv_image = cv2.cvtColor(frame, cv2.COLOR_BGR2HSV_FULL)
            mask_image = cv2.inRange(hsv_image, hsv_min, hsv_max)

            hsv_min = (cv2.getTrackbarPos("H2min", "HSV"), hsv_min_limit[1], hsv_min_limit[2])
            hsv_max = (cv2.getTrackbarPos("H2max", "HSV"), hsv_max_limit[1], hsv_max_limit[2])
            mask_image |= cv2.inRange(hsv_image, hsv_min, hsv_max)

            cv2.imshow("image", frame)
            cv2.imshow("HSV", mask_image)
            cv2.waitKey(1)
